...
\lstdefinestyle{sqlstyle}{
  language=SQL,
  basicstyle=\small\ttfamily,   % моноширинный шрифт для стабильной ширины
  columns=fullflexible,         % гибкая ширина, меньше overfull
  keepspaces=true,              % сохранять пробелы (в т.ч. отступы)
  breaklines=true,
  ...
  captionpos=t,
  upquote=true,                 % прямые кавычки в моно-шрифте
}

% Если языка SQL нет, можно слегка подсветить ключевые слова:
\lstdefinelanguage{SQL}{
  morekeywords={
    CREATE, TABLE, ALTER, ADD, CONSTRAINT, PRIMARY, KEY, FOREIGN, REFERENCES,
    UNIQUE, CHECK, DEFAULT, NOT, NULL, ON, DELETE, CASCADE, RESTRICT,
    INDEX, CREATE INDEX, TIMESTAMPTZ, BIGSERIAL, BIGINT, BOOLEAN, TEXT, VARCHAR,
    SMALLINT
  },
  sensitive=false
}

% Чуть больше «запаса» для переноса текста вне листингов
\setlength{\emergencystretch}{3em}

\captionsetup[lstlisting]{singlelinecheck=false, justification=raggedright}

\usepackage{microtype}
\Urlmuskip=0mu plus 1mu     % разрешить "тонкие" переносы в url/tt-подобных местах

% Чуть больше свободы для переносов в тексте
\setlength{\emergencystretch}{3em}